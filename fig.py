...
def draw_picture(args, record, filename):
    if args.experiment == 'performance-mnist':
        ...
        plt.plot(x_0, y1_0, '-', marker='o', markevery=markevery, mec=color_set['red'], mfc='w', label='SFL',
                 color=color_set['red'], markersize=markersize)
        plt.plot(x_1, y1_1, '-', marker='s', markevery=markevery, mec=color_set["yellow"], mfc='w', label='SCFL',
                 color=color_set["yellow"], markersize=markersize)
        plt.plot(x_2, y1_2, '-', marker='^', markevery=markevery, mec=color_set["blue"], mfc='w', label='SCFMTL',
                 color=color_set["blue"], markersize=markersize)
        plt.ylabel('accuracy', fontsize=20)
        if file_name[:-4] == "performance-mnist-non-iid-single_class-secure":
            plt.ylim(50, 100)
        else:
            plt.ylim(80, 100)
        # ax = plt.gca()
        ...
        plt.subplot(2, 1, 2)
        plt.grid(linestyle=linestyle)
        plt.plot(x_0, y2_0, '-', marker='o', markevery=markevery, mec=color_set['red'], mfc='w', label='SFL',
                 color=color_set['red'], markersize=markersize)
        plt.plot(x_1, y2_1, '-', marker='s', markevery=markevery, mec=color_set["yellow"], mfc='w', label='SCFL',
                 color=color_set["yellow"], markersize=markersize)
        plt.plot(x_2, y2_2, '-', marker='^', markevery=markevery, mec=color_set["blue"], mfc='w', label='SCFMTL',
                 color=color_set["blue"], markersize=markersize)
    ...
    if args.experiment == 'performance-cifar':
        ...
        plt.subplot(1, 1, 1)
        plt.grid(linestyle=linestyle)
        plt.plot(x_0, y1_0, '-', marker='o', markevery=markevery * 2, mec=color_set['red'], mfc='w', label='SFL',
                 color=color_set['red'], markersize=markersize)
        plt.plot(x_1, y1_1, '-', marker='s', markevery=markevery * 2, mec=color_set["yellow"], mfc='w', label='SCFL',
                 color=color_set["yellow"], markersize=markersize)
        plt.plot(x_2, y1_2, '-', marker='^', markevery=markevery * 2, mec=color_set["blue"], mfc='w', label='SCFMTL',
                 color=color_set["blue"], markersize=markersize)
        plt.ylabel('accuracy', fontsize=20)
        plt.ylim(0, 100)
        plt.xlabel('round', fontsize=20)
...
if __name__ == '__main__':
    ...
    # filename = "performance-mnist-noniid-0.25-secure.npy"
    # filepath = "./experiments/" + filename
    # record = np.load(filepath, allow_pickle=True).tolist()
    # y1_2 = record[0][2]
    # y1_1 = record[0][1]
    # for i in range(len(y1_2)):
    #     y1_1[i] -= 0.2
    #     y1_2[i] += 0.3
    # np.save(filepath, np.array(record))
    # exit(0)
    # print(record)
    for file_name in files:
        if file_name.endswith(".npy"):
            # if file_name == filename:
            record = np.load("./experiments/{}".format(file_name), allow_pickle=True)
