...
def Local_Update2(args, w, dataset=None, ids=None, round=0, name="fc1.weight"):
    dataloader = DataLoader(DatasetFolder(dataset, ids), batch_size=args.num_batch, shuffle=True)
    ...
    delta = net.state_dict()[name] - w[name]

    mem = sum([param.nelement() * param.element_size() for param in net.parameters()])
    mem *= 1e-6

    return delta, mem, net.state_dict(), sum(ep_loss) / len(ep_loss)
